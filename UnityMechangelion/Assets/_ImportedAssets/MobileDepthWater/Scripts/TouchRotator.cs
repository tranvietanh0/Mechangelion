namespace Assets.MobileOptimizedWater.Scripts
{
    ...
    public class TouchRotator : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            {
                var deltaPos = Input.mousePosition - this.prevMousePos;

                var deltaRotation = new Vector3(-deltaPos.y, deltaPos.x) * Time.deltaTime * this.mouseSpeed;
                var rotation      = this.cameraRoot.eulerAngles + deltaRotation;

                this.cameraRoot.eulerAngles = new Vector3(Mathf.Clamp(rotation.x, 0f, 90f), rotation.y);
            }
