...
- **After any skill file updated**: ALWAYS invoke `/t1k:sync-back` skill (NEVER manually copy files to kit repos). The skill handles routing, frontmatter, module paths, and PR creation automatically
- **After discovering a skill/agent bug**: ALWAYS invoke `/t1k:issue` skill to report to owning kit repo (NEVER manually create issues)
...
5. **Sync-back check**: if any `$HOME/.claude/skills/` files modified, ALWAYS invoke `/t1k:sync-back` skill (NEVER manually copy)
...
- **Sync-back audit**: `git diff --name-only HEAD~10 | grep '$HOME/.claude/skills/'` — if any changed and not synced, run `/t1k:sync-back` now
