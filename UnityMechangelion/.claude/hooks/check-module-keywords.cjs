...
try {
  ...
  let prompt = '';
  try { prompt = fs.readFileSync('/dev/stdin', 'utf8').trim().toLowerCase(); } catch { /* ok */ }
  ...
  if (fs.existsSync(metaPath)) {
    try {
      ...
      const modules = meta.installedModules || meta.modules || {};
      for (const name of Object.keys(modules)) installed.add(name);
    } catch { /* ok */ }
  }

  // Fallback: also check .t1k-module-summary.txt
  const summaryPath = path.join(cwd, '.t1k-module-summary.txt');
