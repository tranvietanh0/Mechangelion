...
try {
  ...
  function readText(p) { try { return fs.readFileSync(path.join(cwd, p), 'utf8').trim(); } catch { return null; } }
  ...
  const installedSet = new Set(Object.keys(meta?.installedModules || meta?.modules || {}));
  if (registry?.modules) {
  ...
  if (summary) {
    ...
    if (fallbackLines.length > 0) console.log(fallbackLines.join('\n'));
  }
