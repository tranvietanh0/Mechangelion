...
(async () => {
  try {
    ...
    // Skip if no command or if it's a read-only/status command
    if (!cmd) {
      process.exit(0);
    }
    ...
    // Build telemetry entry — no absolute paths, no secrets
    const entry = {
    ...
    const filePath = path.join(telemetryDir, `errors-${date}.jsonl`);
    ...
    if (errorCount >= ERROR_THRESHOLD) {
      // Check if we already triggered watzup for this threshold
